          ...
          <tbody>
            @foreach($coupons as $coupon)
                <tr>
                    ...
                    <td>
                        @if($coupon->isActive())
                            <span class="badge badge-success mb-2">{{$coupon->currentStatus()}}</span>
                            <small class="d-block">
                            @if($coupon->start_date)
                                since {{ $coupon->start_date }}
                            @endif
                            @if($coupon->end_date)
                                until {{ $coupon->end_date }}
                            @endif
                            </small>
                        @elseif($coupon->isInactive())
                            <span class="badge badge-warning">{{$coupon->currentStatus()}}</span>
                            <small class="d-block">
                                @if($coupon->start_date)
                                    active at {{ $coupon->start_date }}
                                @endif
                                @if($coupon->end_date)
                                    until {{ $coupon->end_date }}
                                @endif
                        ...
                        @else
                            <span class="badge badge-danger">{{$coupon->currentStatus()}}</span>
                            <small class="d-block">
                                @if($coupon->start_date)
                                    since {{ $coupon->start_date }}
                                @endif
                                @if($coupon->end_date)
                                    end at {{ $coupon->end_date }}
                                @endif
                            </small>
                        @endif
                    </td>
