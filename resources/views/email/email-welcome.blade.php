...
<head>
    <title>Welcome to {{ $settings->brand_name }}</title>
            ...
            <div class="w-full header-brand color-primary">
                <a href="{{ env('APP_URL') }}">
                    @if($settings->brand_logo)
                        <img src="{{ $brandLogo }}" alt="{{ $settings->brand_name }}" height="100%">
                    @else
                        {{ $settings->brand_name }}
                    @endif
                </a>
            ...
            <div class="mx-auto max-w-md">
                <div class="divide-y divide-gray-300/50">
                    <div class="space-y-6 py-8 text-base leading-5 text-gray-600">
                        <h3 class="font-bold text-xl mb-2">Welcome to {{ $settings->brand_logo }}</h3>
                            <div class="mt-4">
                                <p class="text-xs text-gray-500">
                                    Happy! You have successfully created an account. Come on, log in anytime and anywhere for more comfortable shopping activities.
                                </p>
                            </div>
                            <div class="mb-6 flex items-center justify-center">
                                <a href="{{ route('verification.verify', ['id' => $user->id, 'hash' => $token]) }}" class="color-primary-button px-2 rounded text-white text-sm py-1.5 mb-4">
                                    Confirm Account
                                </a>
                            </div>
                        {{--                    <img src="https://fakeimg.pl/650x60/A8DF8E/FFF/?text=LunarStore&font=lobster" class="h-fit w-full"/>--}}
                        <div class="w-full header-brand color-primary">
                            <a href="{{ env('APP_URL') }}">
                                @if($settings->brand_logo)
                                    <img src="{{ $brandLogo }}" alt="{{ $settings->brand_name }}" height="100%">
